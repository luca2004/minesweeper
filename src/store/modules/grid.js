...
const actions = {
  ...
  help4Flag({ commit, state, dispatch }){
    if(state.nbHelp <= 0) return;
    for (let i = 0; i < state.pattern.length; i++){
      for (let j = 0; j < state.pattern[i].length; j++){
        let row = state.pattern[i][j];
        if(row.bomb === true && row.flagged === false){
          dispatch('flagCell', {row:i, col:j});
          commit("decHelps");
          return;
        }
      }
